...
module measure_distance(pt1, pt2, point_offset, text_offset, units
  ...
  translate(pt1) {
    rotate([arrow_angle,0,0]) translate([0,point_offset,0]) {
      % linear_extrude(height=arrow_head_diameter/4,center=true) arrow_2d(arrow_head_diameter=arrow_head_diameter,arrow_head_height=arrow_head_height,arrow_tail_width=arrow_head_diameter/4,arrow_tail_length=text_offset,arrow_type="left");
    }
  }
  translate(pt2) {
    rotate([arrow_angle,0,0]) translate([0,point_offset,0]) {
      % linear_extrude(height=arrow_head_diameter/4,center=true) arrow_2d(arrow_head_diameter=arrow_head_diameter,arrow_head_height=arrow_head_height,arrow_tail_width=arrow_head_diameter/4,arrow_tail_length=text_offset,arrow_type="right");
    }
  }
  midpt=[pt1[0]+(pt2[0]-pt1[0])/2,pt1[1]+(pt2[1]-pt1[1])/2+point_offset+text_offset,pt1[2]+(pt2[2]-pt1[2])/2];
  translate(midpt) rotate([0,0,-90]) % 8bit_str(chars=units,char_count=len(units),block_size=2,height=4);
}
