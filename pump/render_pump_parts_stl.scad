...
//  import_dxf(file="build/drive_gear_w_hub_holes.dxf");
  import_stl(file="build/drive_gear_w_hub_holes.stl");
}
...
module drive_gear_stack(h=sheet_th) {
  ...
  translate([0,0,3*h+3*stack_space]) drive_gear_w_hub_holes();
  ...
  translate([0,0,6*h+6*stack_space+tube_layer_th]) drive_gear_w_hub_holes();
...
//  import_dxf(file="build/outer_gear_w_mount_holes.dxf");
  import_stl(file="build/outer_gear_w_mount_holes.stl");
}
...
module outer_gear_stack(h=sheet_th) {
  ...
  translate([0,0,3*h+3*(1-expand_gears)*stack_space]) outer_gear_w_mount_holes();
  ...
  if(expand_gears==0) translate([0,0,6*h+6*(1-expand_gears)*stack_space+tube_layer_th]) outer_gear_w_mount_holes();
...
//  import_dxf(file="build/roller_gear_w_hub_holes.dxf");
  import_stl(file="build/roller_gear_w_hub_holes.stl");
}
...
module roller_gear_stack(index=0,h=sheet_th) {
	...
	translate([0,0,3*h+3*stack_space]) roller_gear_w_hub_holes();
	...
	translate([0,0,6*h+6*stack_space+tube_layer_th]) roller_gear_w_hub_holes();
